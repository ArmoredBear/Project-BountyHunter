...
public partial class Phantom : CharacterBody2D
{
    ...
    [Export] public PackedScene SpiritBloodScene;
    [Export] public PackedScene SpiritBloodSplatScene;
    ...
    {
        if (SpiritBloodScene != null)
        {
            GpuParticles2D bloodInstance = SpiritBloodScene.Instantiate<GpuParticles2D>();
            bloodInstance.GlobalPosition = GlobalPosition;
            GetTree().CurrentScene.AddChild(bloodInstance);

            bloodInstance.Emitting = true;
        }

        if (SpiritBloodSplatScene != null)
        {
            GpuParticles2D splat = SpiritBloodSplatScene.Instantiate<GpuParticles2D>();
            splat.GlobalPosition = GlobalPosition;
            GetTree().CurrentScene.AddChild(splat);
